...
public class ConnectionDB {
	...
	public void updateByUsn() {
		try {
			...
			try {
				if (usn.length() <= 4)
					throw new Exception("Please Enter Minimum 4 Character");
			} catch (Exception e) {
				e.printStackTrace();
			}
	...
	public void searchByUsn() throws Exception {
		...
		try {
			...
			try {
				if (usn.length() <= 4)
					throw new Exception("Length is not full fil Please enter min 4 character");
			} catch (Exception e) {
				e.printStackTrace();
			}
